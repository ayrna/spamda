...
\begin{document}
	...
	\begin{frontmatter}
		...
		\title{SPAMDA: A software tool to manage meteorological data for environmental modelling}
		
		\author[UCO]{Antonio M. G\'omez-Orellana\corref{AMGO}}
		\cortext[AMGO]{Corresponding author.}
		\ead{am.gomez@uco.es}
		
		\author[UCO]{Juan C. Fern\'andez\corref{JCFC}}
		\cortext[JCFC]{Corresponding author.}
		\ead{jfcaballero@uco.es}
		
		\author[UCO]{Manuel Dorado-Moreno}
		\ead{manuel.dorado@uco.es}
		
		\author[UCO]{Pedro A. Guti\'errez}
		\ead{pagutierrez@uco.es}
		...
		\begin{abstract}
			Meteorological data play an important role for the comprehension of the environment, and they are extensively used to perform environmental learning. Machine Learning (ML) techniques, a valuable support in many research areas, require datasets containing information related to the topic under study, which are not always available in an appropriate format, and its preparation implies a lot of time and effort by the researchers. This paper presents a software for creating datasets using meteorological observations from two well-known sources of information: the \textit{National Data Buoy Center} and the \textit{National Centers for Environmental Prediction and for Atmospheric Research Reanalysis Project}. As the designed software is able to simplify the creation of the datasets (ready to be used as inputs for ML techniques in prediction tasks) and reduce the time needed for this task, researchers can benefit from it in order to achieve a more efficient exploitation and protection of environmental resources.
		\end{abstract}

		\begin{keyword}
			Meteorological data\sep Reanalysis data\sep Pre-processing\sep Creating datasets\sep Environmental prediction\sep Marine energy
		\end{keyword}
	...
	\section{Introduction}
		...
		\begin{itemize}
			...
			\item It makes the researcher focus on environmental modelling, without having to worry about mechanical tasks.
			\item It provides information about the quality and quantity of the data.
			\item It avoids possible researcher errors in the intermediate steps of the process of creation of the datasets, such as time and geographical coordinates conversion or missing values handling, among others.
			...
			\item Its modular design allows the implementation of new functional modules for managing meteorological data from others sources for environmental research.
